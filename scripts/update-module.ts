...
export const updateModule = async (modulePath: string, moduleName: string) => {
  ...
  const moduleFilePath = path.join(__dirname, '../src/core-service', modulePath, `${moduleName}.module.ts`);
  ...
  try {
    ...
    const moduleDir = path.join(__dirname, '../src/core-service', modulePath);

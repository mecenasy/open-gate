...
export class GetAllByPermissionHandler
  implements IQueryHandler<GetAllByPermissionQuery, { data: CommandProto[]; total: number }>
{
  constructor(private readonly commandService: CommandService) {}

  async execute(query: GetAllByPermissionQuery): Promise<{ data: CommandProto[]; total: number }> {
    const { commands, total } = await this.commandService.findAllByPermission(query.roleName, query.page, query.limit, query.activeOnly);

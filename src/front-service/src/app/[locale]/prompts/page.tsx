...
function UserTypeBadge({ value, label }: { value: string; label: string }) {
  const cfg = USER_TYPE_BADGE[value] ?? {
    dot: 'bg-slate-500',
    pill: 'bg-slate-500/20 text-slate-400 light:text-slate-600 border border-slate-400/50',
  };
  return (
...
export default function PromptsPage() {
  ...
  useEffect(() => {
    if (selectedPrompt) {
      reset({
        ...
        commandName: selectedPrompt.commandName,
  ...
  useEffect(() => {
    if (fullPromptText) {
      setValue('prompt', fullPromptText);
    }
  }, [fullPromptText, setValue]);

  const onSubmitAdd = async (data: PromptForm) => {
  ...
  const columns: TableColumn<PromptSummary>[] = [
    ...
    { key: 'commandName', header: t('colCommandName') },
    {
      ...
      render: (val) => (
        <UserTypeBadge
          value={val as string}
          label={t(USER_TYPE_LABEL_KEY[val as string] as Parameters<typeof t>[0])}
        />
      ),
    },
    {
      ...
      render: (val) => (
        <span onClick={(e) => e.stopPropagation()}>
          <Button
            variant="red"
            size="sm"
            onClick={() => onRemovePrompt(val as string)}
          >
            {t('delete')}
          </Button>
        </span>
      ...
      >
        ...
        >
          <div className="grid grid-cols-3 gap-4 shrink-0">
            <Input
              label={t('fieldKey')}
              error={errorsAdd.key?.message}
              {...registerAdd('key', { required: t('required') })}
            />
            <Input
              label={t('fieldCommandName')}
              error={errorsAdd.commandName?.message}
              {...registerAdd('commandName', { required: t('required') })}
            />
          ...
          <Input
            ...
            {...registerAdd('description', { required: t('required') })}
          />
          <Textarea
            grow
            label={t('fieldPrompt')}
            error={errorsAdd.prompt?.message}
            {...registerAdd('prompt', { required: t('required') })}
          />
        </form>
      </Modal>
        ...
        footer={
          <>
            ...
            <Button
              ...
              type="submit"
              disabled={isUpdatingPrompt || fullPromptLoading}
            >
        ...
        >
          <div className="grid grid-cols-3 gap-4 shrink-0">
            <Input
              ...
              {...register('key', { required: t('required') })}
            />
            <Input
              label={t('fieldCommandName')}
              error={errors.commandName?.message}
              {...register('commandName', { required: t('required') })}
            />
            <Select<PromptUserType>
              label={t('fieldUserType')}
          ...
          {fullPromptLoading ? (
            ...
            <Textarea
              ...
              {...register('prompt', { required: t('required') })}
            />
          )}
        </form>
      </Modal>

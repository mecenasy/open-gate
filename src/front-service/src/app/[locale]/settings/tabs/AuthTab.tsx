...
function PasskeyRow({ deviceName, addedLabel, isCurrent, currentLabel, removeLabel, onRemove }: PasskeyRowProps) {
  return (
...
export function AuthTab() {
  ...
  const [keyToRemove, setKeyToRemove] = useState<{ id: string; credentialID: string; deviceName: string } | null>(null);
  ...
  const isCurrentDevice = (credentialID: string) =>
    typeof window !== 'undefined' && window.localStorage.getItem(`webauthn_${credentialID}`) === 'true';
  const toEvent = (checked: boolean) => ({ target: { checked } }) as React.ChangeEvent<HTMLInputElement>;
  ...
  return (
    <div className="max-w-2xl mx-auto">
            ...
            {passkeys.keys.map((key) => {
              ...
              return (
                <PasskeyRow
                  ...
                  onRemove={() => setKeyToRemove({ id: key.id, credentialID: key.credentialID, deviceName: key.deviceName || 'Passkey' })}
                />
              );
            })}
          </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="blue" size="sm" onClick={closeRemoveModal}>{t('cancel')}</Button>
            <Button variant="red" size="sm" onClick={confirmRemove}>{t('remove')}</Button>
          </>
        }
      >

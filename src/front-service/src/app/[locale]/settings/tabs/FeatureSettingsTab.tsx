...
function FeatureConfigModal({ featureKey, onClose }: FeatureConfigModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all(
        configs.map((cfg) =>
          updateConfig({ variables: { input: { key: cfg.key, value: values[cfg.key] ?? cfg.value } } }),
        ),
      );
      onClose();
    } finally {
        ...
        <div className="flex flex-col gap-5 overflow-y-auto max-h-[60vh]">
          {configs.map((cfg) => (
            <div key={cfg.key} className="flex flex-col gap-1.5">
              ...
              />
              {cfg.description && <p className="text-xs text-muted leading-relaxed">{cfg.description}</p>}
            </div>
          ))}
        </div>
      )}
